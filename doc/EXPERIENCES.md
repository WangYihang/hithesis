...
本文将会介绍笔者在完成毕业设计（论文）中期报告中遇到的一些坑以及发现的可以提高效率的方法。
...
2. minted  
    ...
    另外 Minted 的功能更加全面，例如添加行号、高亮制定行以及设置行号的起始值等（具体可参考文档 `texdoc **minted**`）。
...
笔者暂时未找到合适的伪代码宏包，可临时使用流程图（关于如何插入流程图请见后文）代替。

### 为什么我插入的表格/图片没有乖乖呆在我插入的地方呢

导致这个问题的原因是 LaTeX 复杂的浮动体布局算法。
解决方案是：

1. 忘记在 word 中的 `如下图`、`如上表` 的说法。
2. 使用 `\label` 与 `\ref` 的方式来引用表格/图片。
3. 不去管它们，布局在哪儿就是哪儿，拥抱 LaTeX 的浮动体布局算法。
